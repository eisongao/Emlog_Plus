...
if ($options_cache['isxmlrpcenable'] == 'n') {
	error_message(500, langs('xmlrpc_is_off'));
}
if (!$HTTP_RAW_POST_DATA) {
	error_message(500, langs('xmlrpc_only_post'));
}
...
if (trim($data) == '') {
	error_message(500, langs('error_data_empty'));
}
...
function mw_getPost($args) {
	...
	if (empty($post)) return error_message(404, langs('post_no_access'));
...
function mw_getRecentPosts($args) {
	...
	if (empty($xml)) {
		error_massage(404, langs('no_posts'));
	}
	$xml = "<array><data>$xml</data></array>";
	response($xml);
}
...
function mw_newMediaObject($args) {
	...
	error_message(500, langs('file_error'));
	$filename = $matches[0];
	...
	if (empty($filename))
	error_message(500, langs('file_name_error'));

	$extension = strtolower(substr(strrchr($filename, "."), 1));
	// 文件类型检测
	if (!in_array($extension, $att_type)) {
		error_message(500, langs('file_type_error'));
    }
	...
	if (!is_dir($uppath_root)) {
		...
		if ($ret === false) {
error_message(500, langs('upload_folder_create_error'));
		}
	}
	if (!is_dir($uppath)) {
		umask(0);
		...
		if ($ret === false) {
			error_message(500, langs('upload_folder_unwritable'));
		}
	}

	$fp = @fopen($attachpath, 'wb');
	if (!$fp)
       error_message(500, langs('file_write_error'));
	fwrite($fp, $bits);
	fclose($fp);
...
error_message(403, langs('username_password_error'));
		return false;
	}
	// 返回用户信息
