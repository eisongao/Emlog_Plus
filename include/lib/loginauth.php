...
class LoginAuth{
    ...
    public static function loginPage($errorCode = NULL) {
        ...
        $ckcode = "<span>".langs('captcha')."</span>
        <div class=\"val\"><input name=\"imgcode\" id=\"imgcode\" type=\"text\" />
        <img src=\"../include/lib/checkcode.php\" align=\"absmiddle\"  style=\"cursor : pointer;\" alt=\"".langs('view_code_no')."\" title=\"".langs('refresh_code')."\"  onclick=\"this.src=this.src+'?'\" /></div>" :
        $ckcode = '';
        $error_msg = '';
        if ($errorCode) {
            switch ($errorCode) {
                case self::LOGIN_ERROR_AUTHCODE:
                    $error_msg = langs('captcha_error_reenter');

                    break;
                case self::LOGIN_ERROR_USER:
                    $error_msg = langs('user_name_wrong_reenter');
                    break;
                case self::LOGIN_ERROR_PASSWD:
                    $error_msg = langs('password_wrong_reenter');
                    break;
            }
        }
  ...
  /**
    ...
    public static function checkToken(){
        ...
        if ($token != self::genToken()) {
            emMsg(langs('no_permission').' '.langs('token_error'));
        }
    }
}
