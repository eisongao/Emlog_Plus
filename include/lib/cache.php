...
class Cache {
    ...
    private function mc_record() {
        ...
        while ($show_record = $this->db->fetch_array($query)) {
            ...
            if ($record != $f_record) {
             ...
             'record' => gmdate(langs('cache_date_format'), $show_record['date']),
	...
	function cacheWrite ($cacheData, $cacheName) {
		...
		@ $fp = fopen($cachefile, 'wb') OR emMsg(langs('cache_read_error'));
		@ $fw = fwrite($fp, $cacheData) OR emMsg(langs('cache_not_writable'));
