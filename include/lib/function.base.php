...
define('EMLOG_LANGUAGES',Option::get('language')); 
...
function changeFileSize($fileSize) {
	if ($fileSize >= 1073741824) {
		...
		$fileSize = $fileSize . langs('_bytes');
	}
	return $fileSize;
}
...
function pagination($count, $perlogs, $page, $url, $anchor = '') {
	...
	if ($page > 6)
		$re = "<a href=\"{$urlHome}$anchor\" title=\"".langs('first_page')."\">&laquo;</a><em>...</em>$re";
	if ($page + 5 < $pnums)
		$re .= "<em>...</em> <a href=\"$url$pnums$anchor\" title=\"".langs('last_page')."\">&raquo;</a>";
	if ($pnums <= 1)
		$re = '';
	return $re;
}
...
function breakLog($content, $lid) {
	...
	if (!empty($ret[1])) {
		$ret[0].='<p class="readmore"><a href="' . Url::log($lid) . '">'.langs('read_more').'</a></p>';
        return $ret[0];
	} elseif(Option::get('isexcerpt') == 'y') {
        return subString(trim(strip_tags($content)), 0, Option::get('excerpt_subnum')) . '<p class="readmore"><a href="' . Url::log($lid) . '">'.langs('read_more').'</a></p>';
    } else {
        return $content;
    }
}
...
function smartDate($datetemp) {
   $ptime= $datetemp;
    $etime = time() - $datetemp;
    if ($etime < 1) return langs('just_ago');
    $interval = array (
    12 * 30 * 24 * 60 * 60 => langs('_year_ago').'('.(date('Y-m-d', $ptime)).')',
    30 * 24 * 60 * 60 => langs('_month_ago').'('.(date('m-d', $ptime)).')',
    7 * 24 * 60 * 60 => langs('_week_ago').'('.(date('m-d', $ptime)).')',
    24 * 60 * 60 => langs('_day_ago'),
    60 * 60 => langs('_hour_ago'),
    60 => langs('_min_ago'),
    1 => langs('_sec_ago'),
    );
...
function uploadFile($fileName, $errorNum, $tmpFile, $fileSize, $type, $isIcon = false, $is_thumbnail = true) {
	...
	switch ($result) {
		case '100':
			emMsg(langs('file_size_exceeds_system') . ini_get('upload_max_filesize') . langs('_limit'));
			break;
		case '101':
			emMsg(langs('upload_failed_error_code') . $errorNum);
			break;
		case '102':
			emMsg(langs('file_type_not_supported'));
			break;
		case '103':
			$ret = changeFileSize(Option::getAttMaxSize());
			emMsg(langs('file_size_exceeds_') . $ret . langs('_of_limit'));
			break;
		case '104':
			emMsg(langs('upload_folder_create_error'));
			break;
		case '105':
			emMsgs(langs('upload_folder_unwritable'));
			break;
		default:
			return $result;
			break;
 ...
 * @param $g
 */
if (Option::get('avatar_cache') == 'y'){
function getGravatar($email,$size = '64',$default = '',$alt = 'gravatar'){
  $f = md5(strtolower($email));
  $w = BLOG_URL;
  $a = $w.'avatar/'.$f.'.jpg';
  $e = EMLOG_ROOT .'/avatar/'.$f.'.jpg';
  $t = 1209600; //设定14天
  if (empty($default)) $default = $w. 'admin/views/images/avatar/default_'.rand(1,8).'.png';
  if (!is_file($e) || (time() - filemtime($e)) > $t ){ 
    $r = 'X';
    $g = sprintf( "https://secure.gravatar.com", ( hexdec( $f{0} ) % 2 ) ). '/avatar/'. $f. '?s=64&d='. $default. '&r='. $r;
     copy($g, $e); $a = $g; 
  }
...
EOT;
    if ($url != 'none') {
/*vot*/        echo '<p><a href="' . $url . '">'. langs('click_return').'</a></p>';
    }
    echo <<<EOT
</div>
</body>
...
$emos = array(
array(
'img' => '<span class="icon-smile grin"></span>', 
'title'=>'smile'
),
array(
'img' => '<span class="icon-tongue grin"></span>', 
'title'=>'tongue'
),
array(
'img' => '<span class="icon-sad grin"></span>', 
'title'=>'sad'
),
array(
'img' => '<span class="icon-happy grin"></span>',
'title'=>'oops'
),
array(
'img' => '<span class="icon-smile grin"></span>',
...
array(
'img' => '<span class="icon-wink grin"></span>', 
'title'=>'wink'
),
array(
'img' => '<span class="icon-grin grin"></span>', 
'title'=>'grin'
),
array(
'img' => '<span class="icon-cool grin"></span>', 
'title'=>'cool'
),
array(
'img' => '<span class="icon-angry grin"></span>', 
'title'=>'angry'
),
array(
'img' => '<span class="icon-evil grin"></span>', 
'title'=>'evil'
),
array(
'img' => '<span class="icon-shocked grin"></span>', 
'title'=>'shocked'
),
array(
'img' => '<span class="icon-baffled grin"></span>', 
'title'=>'baffled'
),
array(
'img' => '<span class="icon-confused grin"></span>', 
'title'=>'confused'
),
array(
'img' => '<span class="icon-neutral grin"></span>', 
'title'=>'neutral'
),
array(
'img' => '<span class="icon-hipster grin"></span>', 
'title'=>'hipster'
),
array(
'img' => '<span class="icon-wondering grin"></span>', 
'title'=>'wondering'
),
array(
'img' => '<span class="icon-sleepy grin"></span>', 
'title'=>'sleepy'
),
array(
'img' => '<span class="icon-frustrated grin"></span>', 
'title'=>'frustrated'
),
array(
'img' => '<span class="icon-crying grin"></span>', 
'title'=>'arrow'
),
array(
'img' => '<span class="icon-neutral grin"></span>', 
