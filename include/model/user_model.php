...
class User_Model {
	...
	function getUsers($page = null) {
		...
		while($row = $this->db->fetch_array($res)) {
			...
			$row['website']   = htmlspecialchars($row['website']);
...
function addUser($login, $password,  $email,$website,$role, $ischeck) {
		$sql="insert into ".DB_PREFIX."user (username,password,email,website,role,ischeck) values('$login','$password','$email','$website','$role','$ischeck')";
		$this->db->query($sql);
	}
...
function deleteUser($uid) {
  ...
  function isEmailExist($email, $uid = '') {
         if(empty($email)) {
            return FALSE;
        }
        $subSql = $uid ? 'and uid!='.$uid : '';
        $data = $this->db->once_fetch_array("SELECT COUNT(*) AS total FROM ".DB_PREFIX."user WHERE email='$email' $subSql");
        if ($data['total'] > 0) {
            return true;
        }else {
            return false;
