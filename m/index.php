...
if (Option::get('ismobile') == 'n') {
	emMsg(langs('mobile_version_unavailable'), BLOG_URL);
}
...
if (!empty ($logid)) {
	...
	if ($logData === false) {
		mMsg (langs('blog_entry_not_exists'), './');
	}
	extract($logData);
...
if (ISLOGIN === true && $action == 'savelog') {
	...
	if ('n' == $logData['checked']) {
		mMsg(langs('blog_saved_wait_approve'), './');
	}
	emDirect("./");
}
// 评论
if ($action == 'addcom') {
	...
	if($Comment_Model->isLogCanComment($blogId) === false){
        mMsg(langs('comment_error_comment_disabled'), $targetBlogUrl);
    } elseif ($Comment_Model->isCommentExist($blogId, $name, $content) === true){
        mMsg(langs('comment_error_content_exists'), $targetBlogUrl);
    } elseif ($Comment_Model->isCommentTooFast() === true) {
		mMsg(langs('comment_error_flood_control'), $targetBlogUrl);
	} elseif (strlen($name) > 20 || strlen($name) == 0){
        mMsg(langs('comment_error_name_invalid'), $targetBlogUrl);
    } elseif ($mail != '' && !checkMail($mail)) {
        mMsg(langs('comment_error_email_invalid'), $targetBlogUrl);
    } elseif (ISLOGIN == false && $Comment_Model->isNameAndMailValid($name, $mail) === false){
        mMsg(langs('comment_error_other_user'), $targetBlogUrl);
    } elseif (strlen($content) == '' || strlen($content) > 2000) {
        mMsg(langs('comment_error_content_invalid'), $targetBlogUrl);
    } elseif (ROLE == ROLE_VISITOR && Option::get('comment_needchinese') == 'y' && !preg_match('/[\x{4e00}-\x{9fa5}]/iu', $content)) {
		mMsg(langs('comment_error_national_chars'), $targetBlogUrl);
	}elseif (ISLOGIN == false && Option::get('comment_code') == 'y' && session_start() && $imgcode != $_SESSION['code']) {
        mMsg(langs('comment_error_captcha_invalid'), $targetBlogUrl);
    } else {
		...
		if ($hide == 'n') {
		    ...
		    doAction('comment_saved', $cid);
		    mMsg(langs('comment_wait_approve'), $targetBlogUrl);
		}
    }
}
...
if ($action == 'reply') {
	...
	if(!$commentArray) {
		mMsg(langs('comment_parameter_error'), './');
	}
...
if ($action == 'login') {
	Option::get('login_code') == 'y' ? $ckcode = "<span>".langs('captcha')."</span>
