...
if ($action == '') {
	...
	$tplForEm = !empty($tplForEmlog[1]) ?langs('ok_for_emlog') . $tplForEmlog[1] : '';

	if (isset($tplAuthor[1])) {
		$tplAuthor = !empty($tplUrl[1]) ? langs('user').": <a href=\"{$tplUrl[1]}\">{$tplAuthor[1]}</a>" : langs('user').": {$tplAuthor[1]}";
	} else{
		$tplAuthor = '';
	}
	//模板列表
...
$tplData = implode('', @file(TPLS_PATH.$file.'/header.php'));
			...
			$tplInfo['Author'] = !empty($Url[1]) ? langs('user')."：<a href=\"{$Url[1]}\">{$Author[1]}</a>" :  langs('user')."：{$Author[1]}";
...
if ($action == 'upload_zip') {
	...
	if (!$zipfile || $zipfile['error'] >= 1 || empty($zipfile['tmp_name'])) {
		emMsg(langs('template_upload_failed'));
	}
