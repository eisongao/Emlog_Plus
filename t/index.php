...
if ($action == 'getr') {
    ...
    if ($replys) {
	    foreach($replys as $val){
	    	$sub_reply = Option::get('istreply') == 'y' ? "<a href=\"javascript:re({$tid}, '@".addslashes($val['name'])."：');\">".langs('reply')."</a>" : '';
	    	$response .= "
	         <li>
    	...
    	$response .= "<li>".langs('no_replies')."</li>";
    }
    echo $response;
}
...
if ($action == 'reply') {
    ...
    $response = "
         ...
         <em><a href=\"javascript:re({$tid}, '@{$name}：');\">".langs('reply')."</a></em>
         </li>";
    echo $response;
}
